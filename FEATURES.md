...
- An opt-in isolated honeypot terminal with metadata-only mode by default and
  explicit canonical redacted payload mode; bounded entry count, age,
...
- Local filesystem and hosted HTTP(S) blocklist sources, fetched through
  Proxima's existing HTTP client with bounded streamed bodies, a 30-second
  whole-response timeout, and atomic last-good-snapshot retention on fetch or
  parse failure.
...
- Upstream rebinding protection for private, local, link-local, unspecified,
  ...
  trusted local split-DNS without disabling protection globally. Authenticated
  status reports the protection state and bounded exception count without
