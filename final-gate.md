...
| Unit and integration tests | `cargo test --offline --all-targets` passed at Blackhole `3f5fd91` with 113 tests, 113 passed, including install-failure capture rollback, upstream QR/question/overflow fail-closed behavior, the privacy-safe recording proof, scoped regex policy proof, the real HTTP admin fixture, 4 real loopback UDP/TCP resolver fixtures (including `listener_retries_a_truncated_upstream_reply_over_tcp`), and 13 fake-upstream tests against GitHub Proxima revision `46176f4`. |
...
| Nextest | `cargo nextest run --offline --workspace` passed at Blackhole `3f5fd91`: 113 tests, 113 passed, 0 skipped, against GitHub Proxima revision `46176f4`; the real admin, resolver, and fake-upstream fixtures pass under the parallel runner. |
