...
| Named client groups | `client_groups` provide bounded reusable IPv4/IPv6 CIDR sets for service profiles; `client_groups_assign_one_profile_to_multiple_networks` proves one profile expands across multiple groups, while unknown and mixed direct/group scopes fail closed at Blackhole `0e5a359`. |
