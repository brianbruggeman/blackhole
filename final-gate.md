...
| Performance gate | `cargo run --offline --release --features perf-instrument --example performance_gate` ran three times on Linux x86_64 at source `74ea46a`, rustc 1.98.0, Proxima `0652a0c0`. The five-sample harness reports p50/p95/p99, min/max, CoV, derived throughput, allocations, alloc bytes, RSS, load, and boundary counters. Observed p50 ranges: build 43.22–48.44 ms, match 39.01–39.53 µs, parse 36–48 ns, owned 164–269 ns. Listener TCP/encode/transport boundaries were instrumented but not exercised by this example. No zero-copy or production-performance claim is supported. |
