...
| Unit and integration tests | `cargo test --offline --all-targets` passed at Blackhole `0b27452` with 111 tests, 111 passed, including install-failure capture rollback, upstream-overflow fail-closed behavior, the privacy-safe recording proof, scoped regex policy proof, the real HTTP admin fixture, 4 real loopback UDP/TCP resolver fixtures (including `listener_retries_a_truncated_upstream_reply_over_tcp`), and 11 fake-upstream tests against GitHub Proxima revision `d8cf174e`. |
...
| Nextest | `cargo nextest run --offline --workspace` passed at Blackhole `0b27452`: 111 tests, 111 passed, 0 skipped, against GitHub Proxima revision `d8cf174e`; the real admin, resolver, and fake-upstream fixtures pass under the parallel runner. |
