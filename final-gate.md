...
| Gate | Evidence |
| --- | --- |
| Formatting | `cargo fmt --all -- --check` passed at Blackhole `d930cf1` with Rust `1.98.0`. |
| Compilation | `cargo check --offline --all-targets` passed at Blackhole `3f5fd91` with GitHub Proxima revision `46176f4`, Rust `1.98.0`, and Cargo `1.98.0`. |
| Unit and integration tests | `cargo test --offline --all-targets` passed at Blackhole `89f6ebf` with 117 tests, 117 passed, including named client-group expansion/validation, install-failure capture rollback, upstream QR/question/overflow fail-closed behavior, the privacy-safe recording proof, scoped regex policy proof, the real HTTP admin fixture, 4 real loopback UDP/TCP resolver fixtures (including `listener_retries_a_truncated_upstream_reply_over_tcp`), and 15 fake-upstream tests against GitHub Proxima revision `46176f4`. |
...
| Nextest | `cargo nextest run --offline --workspace` passed at Blackhole `89f6ebf`: 117 tests, 117 passed, 0 skipped, against GitHub Proxima revision `46176f4`; the real admin, resolver, and fake-upstream fixtures pass under the parallel runner. |
...
| Encrypted upstream transport | Blackhole's `udp`, `tcp`, `tls`, and `doh` upstream modes are wired to Proxima's existing DNS client and HTTP/TLS pipe adapters. Encrypted modes require an explicit validated server name; Proxima's GitHub `46176f4` tests prove stream-only DoT and HTTP-pipe DoH, while Blackhole's configuration validation, real listener suite, and `c028831` policy-to-DoH/cache plus malformed-payload fixtures pass without a UDP probe. |
...
| Performance gate | The expanded scalar harness ran three times (`N=25` samples per workload per run) with `cargo run --offline --release --example performance_gate --features perf-instrument` at Blackhole `6dd81e4`, GitHub Proxima `46176f4`, Rust `1.98.0`, Linux x86_64, and load average 2.50–2.63. All values below are MEASURED by the running harness: build p50 46.009–49.389 ms with 250,025 allocations / 27,722,250 bytes per run; match p50 40.466–41.544 µs with 2,500 allocations / 57,500 bytes; parse-short p50 52–53 ns, parse-long 231–239 ns, parse-adversarial 78–82 ns, parse-mixed 81–84 ns, and owned 117–124 ns with 50 allocations / 400 bytes. The instrumented boundaries recorded policy canonicalization 60,000 bytes and borrowed-to-owned 300 bytes; this isolated harness did not exercise TCP frame, encode, or transport boundaries (0 recorded). Short/mixed/owned CoV remained high (0.53–1.32), so this is measurement evidence only; no zero-copy or production-performance claim is supported. |
...
| Incumbent parity | Pi-hole/AdGuard parity is incomplete: DoQ, admin/API/UI, DHCP, and richer policy controls remain open; DNS-over-TLS/HTTPS upstreams and named service profiles are implemented and tested. |
