...
| Compilation | `cargo check --offline --all-targets` passed with GitHub Proxima revision `d8cf174e`, Rust `1.98.0`, and Cargo `1.98.0`. |
| Unit and integration tests | `cargo test --offline --all-targets` passed with 109 tests, 109 passed, including the privacy-safe recording proof, scoped regex policy proof, the real HTTP admin fixture, 4 real loopback UDP/TCP resolver fixture tests (including `listener_retries_a_truncated_upstream_reply_over_tcp`), and 10 fake-upstream tests against GitHub Proxima revision `d8cf174e`. |
...
| TCP upstream fallback | `DnsClientUpstream` now retains the echoed question and TC bit, validates `QR=1`/`OPCODE=0`, retries TC-marked UDP answers through the injected existing `StreamUpstream`, and exchanges bounded two-byte-length DNS-over-TCP frames under the same configured deadline. Proxima’s deterministic fallback test and Blackhole’s full all-target suite pass against GitHub Proxima `d8cf174e`. |
| Performance gate | The five-sample scalar harness ran on Linux x86_64 at Blackhole `08e8251`, Rust `1.98.0`, and GitHub Proxima `d8cf174e`; it measured build p50 47.055 ms (CoV 0.006758, 50,005 allocations), match p50 40.026 µs (CoV 0.019285, 500 allocations), parse p50 37 ns (CoV 1.096798), and owned p50 168 ns (CoV 0.963404). The actual listener forwarding fixture separately recorded policy canonicalization 28 B, borrowed-to-owned 28 B, TCP frame buffer 31 B, encode output 120 B, and transport write 120 B. Parse/owned variance is high at N=5, so this is measurement evidence only; no zero-copy or production-performance claim is supported. |
