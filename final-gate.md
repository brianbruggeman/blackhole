...
| Gate | Evidence |
| --- | --- |
| Formatting | `cargo fmt --all -- --check` passed at Blackhole `81ca65f` with Rust `1.98.0`. |
| Compilation | `cargo check --offline --all-targets` passed at Blackhole `81ca65f` with GitHub Proxima revision `e1c91f7`, Rust `1.98.0`, and Cargo `1.98.0`. |
| Unit and integration tests | `cargo test --offline --all-targets` passed at Blackhole `81ca65f` with 113 tests, 113 passed, including install-failure capture rollback, upstream QR/question/overflow fail-closed behavior, the privacy-safe recording proof, scoped regex policy proof, the real HTTP admin fixture, 4 real loopback UDP/TCP resolver fixtures (including `listener_retries_a_truncated_upstream_reply_over_tcp`), and 13 fake-upstream tests against GitHub Proxima revision `e1c91f7`. |
...
| Nextest | `cargo nextest run --offline --workspace` passed at Blackhole `81ca65f`: 113 tests, 113 passed, 0 skipped, against GitHub Proxima revision `e1c91f7`; the real admin, resolver, and fake-upstream fixtures pass under the parallel runner. |
| Doctests | `cargo test --doc --offline` passed at Blackhole `ce3b7d1`: 1 doctest. |
| No-std and WASM | `cargo build --offline --no-default-features --target thumbv7m-none-eabi` and the equivalent `wasm32-unknown-unknown` command passed at Blackhole `81ca65f` with GitHub Proxima revision `e1c91f7`; both completed without warnings. |
