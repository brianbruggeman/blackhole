...
| Compilation | `cargo check --offline --all-targets` passed with GitHub Proxima revision `e39a42e8`, Rust `1.98.0`, and Cargo `1.98.0`. |
| Unit and integration tests | `cargo test --offline --all-targets` passed with 108 tests, 108 passed, including the privacy-safe recording proof, scoped regex policy proof, the real HTTP admin fixture, 3 real loopback UDP/TCP resolver fixture tests, and 10 fake-upstream tests against GitHub Proxima revision `e39a42e8`. |
...
| TCP upstream fallback | `DnsClientUpstream` now retains the echoed question and TC bit, retries TC-marked UDP answers through the injected existing `StreamUpstream`, and exchanges bounded two-byte-length DNS-over-TCP frames. Proxima’s deterministic fallback test and Blackhole’s full all-target suite pass against GitHub Proxima `e39a42e8`. |
