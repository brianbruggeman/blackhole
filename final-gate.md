...
| Performance gate | `cargo run --offline --release --features perf-instrument --example performance_gate` ran three times on Linux x86_64 at Blackhole `b1fb896`, rustc 1.98.0, and Proxima `0652a0c0`; the artifact included explicit `source_commit=b1fb896` metadata. The five-sample harness reports p50/p95/p99, min/max, CoV, derived throughput, allocations, alloc bytes, RSS, load, and boundary counters. Observed p50 ranges: build 42.99–48.42 ms, match 46.69–48.15 µs, parse 35–37 ns, owned 159–176 ns. Listener TCP/encode/transport boundaries were instrumented but not exercised by this example. No zero-copy or production-performance claim is supported. |
