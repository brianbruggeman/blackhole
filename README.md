...
Rust and the wire/runtime edge is Proxima from `../proxima`; the default Prime
path has no Tokio dependency. DNS over UDP and TCP share one bind.
...
DNS edge. Unmatched queries receive NODATA for compatibility; explicit
`forward` rules use the opt-in Proxima upstream pipe and fail closed when no
upstream is attached.
